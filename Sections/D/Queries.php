...
class Queries
{
    ...
    function pickParts()
    {
        ...
        $queryPartNum = "SELECT partNumber, qty, location FROM parts WHERE location LIKE 'D1%' ORDER BY id;";
        $result = mysqli_query(NewFile::establishConnection(), $queryPartNum);

        echo "<form method='post'>";

        if (mysqli_num_rows($result)) {
            echo "<select name='PickForm'>";
            while ($row = mysqli_fetch_array($result)) {

                echo "<option value='Part'>Part Number: $row[partNumber] &nbsp; Quantity: $row[qty] &nbsp; Location: $row[location]</option>";
            }
            echo "</select>";
        }
        echo "<form method='post'><input type='number' name='quantPicked'><input type='submit' name='qtyPickSubmit'></form>";
